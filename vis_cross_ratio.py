...
def process_data(r, num):
    ...
    eqs = [(x - 1, "orange"),
           ...
           ((x + 0.5) ^ 2 + y ^ 2 - 0.5 ^ 2, "pink"), (eqn, "red")]
    for eq, c in eqs:
    ...
    Xs = np.linspace(start=-1, stop=1, num=num)
    Ys = np.concatenate([np.linspace(start=-1, stop=1, num=num // 2), np.linspace(start=1.01, stop=10, num=num // 2)])
