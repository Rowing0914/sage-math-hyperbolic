...
base_polygon = HyperbolicPolygon([0.622152, 0.545613 * I, -0.622152, -0.545613 * I], model="PD", options={})
...
@cached_function
def process_data(base_polygon, base_pt_x, base_pt_y):
    # base point
    ...
    for i in range(n):
        ...
        print(f"Angle of vertex {i}: {np.round(a=np.rad2deg(float(angle)), decimals=2)}")
...
p_base, sides, reflect_1st_sides, reflect_1st_pBase, reflect_2nd_sides, reflect_2nd_pBase, list_perp_bisec, diff_index, ind = process_data(
    base_polygon, base_pt_x, base_pt_y)


@interact
def _(base_pt_x=base_pt_x, base_pt_y=base_pt_y, auto_update=False,
    ...
    if_rerun = base_pt_x != prev_base_pt_x or base_pt_y != prev_base_pt_y
